...
(function (document) {
  ...
  _c_qsa = function _c_qsa(selector) {
    if (!cache(selector)) {
    ...
    function markHours() {
      ...
      if (_c_qs('#select_date').value) {
        var datePVal = _c_qs('#select_date').value,
            ...
            datePMonth = parseInt(datePVal.slice(3, 5)) - 1,
            // неизвестно почему, но в дату ставится следующий месяц
